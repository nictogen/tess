 ...
 */
class HealOverTimeMove(mainStat: MainStat, type : Move.Type, source: Source, name: String) : Move(mainStat, type, source, name), IOngoingMove {
    ...
    override fun ongoingEffect(user: CombatHandler.CombatParticipant, target: CombatHandler.CombatParticipant, combat: Combat) {
        ...
        val healAmount = Tess.rand.nextInt(getMainStatValue(user)).toDouble() / 4.0 + 1.0
